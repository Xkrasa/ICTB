...
验证链路：image_input → mask_edit(auto_face) → gpt_image → seedance_video
- gpt_image 成功后进入 awaiting_approval
- 用户批准后 gpt_image 继续，触发 seedance_video
- seedance_video 成功后进入 awaiting_approval
- 批准后最终 success

注意：会调用真实的 GPT 图生图 API 和 Seedance 视频生成 API，产生费用。
"""
import os
os.environ['GPT_IMAGE_TIMEOUT'] = '120'  # 用户要求 2 分钟超时
...
def main():
    print('=== 真实 API 批准模式闭环测试 ===')
    print(f'服务地址: {BASE}')
    print('注意：此测试会调用真实 GPT 和 Seedance API，产生费用。')

    img_bytes = create_face_image()
    image_url = upload_image(img_bytes)
    print(f'上传图片: {image_url}')

    canvas_id = 'e2e_approval_chain_real'
    nodes = [
        {'id': 'img', 'type': 'image_input', 'x': 100, 'y': 100, 'data': {'image_url': image_url}},
        {'id': 'mask', 'type': 'mask_edit', 'x': 400, 'y': 100, 'data': {'mask_mode': 'auto_face', 'expand': 0.25}},
